...
public class ThreadPoolTaskExecutor implements TaskExecutor {
	...
	public TaskFuture schedule(Task task, long delay) {
		...
		ScheduledFuture<?> scheduledFuture = executor.schedule(wrapped, delay, TimeUnit.MILLISECONDS);
	...
	public TaskFuture scheduleAtFixedRate(Task task, long delay, long period) {
		...
		taskDetail.nextExecuted = System.currentTimeMillis() + delay;
		final SimpleTask wrapped = new SimpleTask(task, taskDetail);
		ScheduledFuture<?> scheduledFuture = executor.scheduleAtFixedRate(wrapped, delay, period, TimeUnit.MILLISECONDS);
		taskFutures.put(task, new TaskFutureImpl(taskDetail, scheduledFuture));
		return taskFutures.get(task);
	}
	...
	public TaskFuture scheduleWithFixedDelay(Task task, long delay, long period) {
		...
		taskDetail.nextExecuted = System.currentTimeMillis() + delay;
		final SimpleTask wrapped = new SimpleTask(task, taskDetail);
		ScheduledFuture<?> scheduledFuture = executor.scheduleWithFixedDelay(wrapped, delay, period, TimeUnit.MILLISECONDS);
		taskFutures.put(task, new TaskFutureImpl(taskDetail, scheduledFuture));
		return taskFutures.get(task);
	}
	...
	public TaskFuture schedule(Task task, CronExpression cronExpression) {
		...
		ScheduledFuture<?> scheduledFuture = executor.schedule(wrapped, executed - System.currentTimeMillis(), TimeUnit.MILLISECONDS);
		taskFutures.put(task, new TaskFutureImpl(taskDetail, scheduledFuture));
		return taskFutures.get(task);
	}
