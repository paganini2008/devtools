...
public class Bytes {
	...
	public static void asc(byte[] array) {
		if (isNotEmpty(array)) {
	...
	public static void desc(byte[] array) {
		if (isNotEmpty(array)) {
			byte[] aux = (byte[]) array.clone();
	...
	public static byte[] toByteArray(Set<Byte> set) {
	...
	public static Set<Byte> asSet(byte[] array) {
		Assert.isNull(array, "Source array must not be null.");
		Set<Byte> set = new LinkedHashSet<Byte>(array.length);
		for (byte a : array) {
			set.add(Byte.valueOf(a));
		}
		return set;
	...
	public static byte cast(BigInteger value) {
		if (NumberRange.checkByte(value) != 0) {
			throw new NumberOverflowException();
		}
