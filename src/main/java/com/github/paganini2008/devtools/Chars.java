...
public class Chars {
	...
	public static List<Character> asUnmodifiableList(char[] array) {
		return Collections.unmodifiableList(asList(array));
	}

	public static Set<Character> asSet(char[] array) {
		Assert.isNull(array, "Source array must not be null.");
		Set<Character> set = new LinkedHashSet<Character>(array.length);
		for (char a : array) {
			set.add(Character.valueOf(a));
		}
		return set;
	...
	public static Set<Character> asUnmodifiableSet(char[] array) {
