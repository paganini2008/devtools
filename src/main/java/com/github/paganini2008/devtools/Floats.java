...
public class Floats {
	...
	public static void asc(float[] array) {
		if (isNotEmpty(array)) {
	...
	public static void desc(float[] array) {
		if (isNotEmpty(array)) {
			float[] aux = (float[]) array.clone();
	...
	public static float[] toFloatArray(Set<Float> set) {
		Assert.isNull(set, "Source set must not be null.");
		float[] array = new float[set.size()];
		int i = 0;
		for (Float a : set) {
			if (a != null) {
				array[i++] = a.floatValue();
	...
	public static Set<Float> asSet(float[] array) {
		Assert.isNull(array, "Source array must not be null.");
		Set<Float> set = new LinkedHashSet<Float>(array.length);
		for (float a : array) {
			set.add(Float.valueOf(a));
		}
		return set;
